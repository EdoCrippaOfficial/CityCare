...
/** Presenter associated to {@link StarredReportsFragment} */
public class StarredReportsPresenter implements MainContracts.ReportListPresenter {
     ...
     * {@link StarredReportsPresenter#onLoadReportsTaskComplete(QuerySnapshot)} has to be executed */
    private QuerySnapshot results;

    /** This flag is set when {@link StarredReportsPresenter#onUpdateTaskComplete()}
    ...
    public void loadReports() {
        ...
        reader.readStarredReports(FirebaseAuthHelper.getUserId());
    }
        ...
        if (results != null) {
            for (QueryDocumentSnapshot snap: results) {
                ...
                Report report = new Report(id, userId, userName, title, description, reply,
                        operatorId, timestamp, (int) nStars, position, status, true);
